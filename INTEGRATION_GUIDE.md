...
async function getUsers(token) {
    try {
        const response = await axios.get('https://loan-system-api-0knv.onrender.com/api/users/', {
            headers: {
...
async function applyForLoan(token, loanData) {
    try {
        const response = await axios.post('https://loan-system-api-0knv.onrender.com/api/loans/', loanData, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
...
const pool = new Pool({
  connectionString: 'postgresql://loan_sytem_db_user:QqPUj19561h5nMkUFuGRwwkyzAdamUnI@dpg-d61eo14r85hc7399gmu0-a.oregon-postgres.render.com/loan_sytem_db',
